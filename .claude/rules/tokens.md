...
There is no Tailwind and no framework utility classes. Consumers and components use CSS custom properties. Hex, `rgb()`, and raw color literals belong in `devex/src/tokens/` only. `npm run lint` runs `scripts/ban-hex.mjs`.
