...
import { AppShell, Button, Theme } from '@luksha6/devex-ui';
import { AudienceSwitch, Composer, Run, type DocBlock } from '@luksha6/devex-ui/docs';
```

Load `styles.css` once. The JS entry does not apply CSS. Wrap the tree in `Theme` so tokens and the reset apply under a root you own. Overlay primitives (`Dialog`, `Drawer`, `Menu`, `Popover`, `Tooltip`, `CommandPalette`, `Toast`, `Nav`) and `Theme` are client components. Next App Router: `'use client'` on those files. Do not import files from `src/`.

| Export                         | What you get                                                                          |
| ------------------------------ | ------------------------------------------------------------------------------------- |
| `@luksha6/devex-ui/styles.css` | Tokens, reset, compiled modules, Onest. Required once per app.                        |
| `@luksha6/devex-ui`            | Product kit: actions, forms, overlays, layout (`Page`, `AppShell`), data, type, Theme |
| `@luksha6/devex-ui/docs`       | Pages, dual reader, `Composer`, and run UI, plus `renderHuman` / `toAgentMarkdown`    |
| `@luksha6/devex-ui/tokens.css` | Token file only, if you need variables without the compiled modules                   |

`.` is the product kit. `./docs` is pages, the dual reader, and UI for starting and watching a run. People get layout. Agents get markdown from the same `DocBlock[]`. `Composer` starts a job. `Run`, `Stream`, `ToolCall`, and `RunStatus` are for people watching it. Primitives must not import the docs barrel.
...
Foundations: Color, Typography, Space, Motion. Guides: Installation, Dual reader, Layout, Agent surfaces, App shell.
...
| `npm test`           | Vitest                                         |
| `npm run lint`       | ESLint, Stylelint, Prettier, hex ban, contrast |
